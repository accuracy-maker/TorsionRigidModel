...
namespace ctr {
    ...
    std::vector<Section> SectionComputer::computeSections(
        ...
        auto sections = computeBoundaries(beta, num_tubes);

        // step 2: compute resultant curvatures from alpha values
        computeCurvature(sections, alpha);
    ...
    std::vector<Section> SectionComputer::computeBoundaries(const double beta[], int num_tubes) const {
        assert(num_tubes > 0);
        assert(num_tubes <= static_cast<int>(tubes.size()));
        ...
        if (!boundaries.empty()) {
            ...
            for (size_t i = 1; i < boundaries.size(); i++) {
                if (boundaries[i] - unique_boundaries.back() > merge_tolerance_) {
        ...
        for (size_t k = 0; k + 1 < unique_boundaries.size(); k++) {
            ...
            if (s_end - s_start < merge_tolerance_) {
                continue;
            }
    ...
    void SectionComputer::computeCurvature(
        ...
        for (auto& sec : sections) {
            ...
            for (size_t j = 0; j < sec.active_tubes.size(); j++) {
                int tube_idx = sec.active_tubes[j];
                ...
                kappa_wx_sum += EI_i * kx_W;
                kappa_wy_sum += EI_i * ky_W;
                EI_sum += EI_i;
            }
    ...
    int SectionComputer::getSegmentIndexAt(
        ...
        for (size_t j = 0; j < segments.size(); j++) {
            ...
            if (local_s < seg_end + merge_tolerance_) {
